...
static const char* FEE_PRUNESTATES_FILENAME="trace_prunestates.dat";
...
void Shutdown()
{
    ...
    if (fFeeEstimatesInitialized)
    {
        ...
        boost::filesystem::path pst_path = GetDataDir() / FEE_PRUNESTATES_FILENAME;
        CAutoFile est_fileout(fopen(est_path.string().c_str(), "wb"), SER_DISK, CLIENT_VERSION);
	CAutoFile pst_fileout(fopen(pst_path.string().c_str(), "wb"), SER_DISK, CLIENT_VERSION);
        if (!est_fileout.IsNull())
            mempool.WriteFeeEstimates(est_fileout);
        else
            LogPrintf("%s: Failed to write fee estimates to %s\n", __func__, est_path.string());
	if (!pst_fileout.IsNull())
            mempool.WritePruneStates(pst_fileout);
        else
            LogPrintf("%s: Failed to write trace prunestates to %s\n", __func__, pst_path.string());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 14679, 18333) + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    filesystem::path undosDir = GetDataDir() / "undos";
    if (!filesystem::exists(blocksDir))
    {
        filesystem::create_directories(blocksDir);
        filesystem::create_directories(undosDir);
        bool linked = false;
        for (unsigned int i = 1; i < 10000; i++) {
            filesystem::path source = GetDataDir() / strprintf("blk%08u.dat", i);
            if (!filesystem::exists(source)) break;
            filesystem::path dest = blocksDir / strprintf("blk%09u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking blk%09u.dat : %s\n", i, e.what());
                break;
            }
        }
        if (linked)
    ...
    CAutoFile pst_filein(fopen(pst_path.string().c_str(), "rb"), SER_DISK, CLIENT_VERSION);
    // Allowed to fail as this file IS missing on first startup.
    if (!est_filein.IsNull())
        mempool.ReadFeeEstimates(est_filein);
    if (!pst_filein.IsNull())
        mempool.ReadPruneStates(pst_filein);
