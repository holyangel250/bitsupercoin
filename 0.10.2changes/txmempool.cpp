...
bool CTxMemPool::WritePruneStates(CAutoFile& fileout) const{
    try {
        LOCK(cs);
        fileout << 99900; // version required to read: 0.9.99 or later
        ...
        fileout << tracedHeight;
        fileout << refactorHeights.size();
        if(!refactorHeights.empty()){  
            fileout << refactorHeights;
        }
        fileout << pruneStates.size();
        if(!pruneStates.empty()){
            fileout << pruneStates;
        }
        ...
        LogPrintf("CTxMemPool::WritePruneStates() : unable to write PruneStates data (non-fatal)");
        return false;
    }
    return true;
}
...
bool CTxMemPool::ReadPruneStates(CAutoFile& filein){
    try {
        int nVersionRequired, nVersionThatWrote;
        filein >> nVersionRequired >> nVersionThatWrote;
        if (nVersionRequired > CLIENT_VERSION)
            return error("CTxMemPool::ReadPruneStates() : up-version (%d) prunestate file", nVersionRequired);

        LOCK(cs);
        
        int nFileTracedHeight;
        filein >> nFileTracedHeight;
        size_t numRefactors;
        filein >> numRefactors;
        if (numRefactors < 0 || numRefactors > 100){
            throw runtime_error("Corrupt refactorHeights file. Must have between 0 and 100 entries.");
        }
        if(numRefactors > 0){
            std::vector<int> FileRefactorHeights;
	    filein >> FileRefactorHeights;
            refactorHeights.swap(FileRefactorHeights);
	}
        size_t numPrunes;
        filein >> numPrunes;
        if (numPrunes < 0 ){
            throw runtime_error("Corrupt pruneStates file. Not negative entries.");
        }
	if(numPrunes > 0){
            std::vector<unsigned char> FilePruneStates;
	    filein >> FilePruneStates;
            pruneStates.swap(FilePruneStates);
	}
        tracedHeight = nFileTracedHeight;
       
    }
    catch (const std::exception &) {
        LogPrintf("CTxMemPool:: ReadPruneStates() : unable to read prunestates data (non-fatal)");
        return false;
    }
    return true;
}
